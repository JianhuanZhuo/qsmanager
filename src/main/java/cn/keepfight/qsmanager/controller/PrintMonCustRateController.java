 ...
 */
public class PrintMonCustRateController extends PrintTemplate implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        table.fixedCellSizeProperty().bind(table.heightProperty().subtract(50).divide(Bindings.size(table.getItems())));
        id.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getTableView().getItems().indexOf(param.getValue()) + 1));

        table.getItems().setAll(IntStream.range(1, 14)
