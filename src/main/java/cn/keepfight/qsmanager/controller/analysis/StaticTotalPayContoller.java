...
import static cn.keepfight.utils.FXWidgetUtil.addToGridPaneLink;

public class StaticTotalPayContoller implements ContentCtrl, Initializable {
    public VBox root;
    public HBox income;
    ...
    public void showed(Properties params) {
        ...
        try {
            listRaw = StaticTotalServers.staticReceiptAllRaw(year);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void showedAfter(Properties params) {
        ...
        addToGridPane(gridPane, "采购材料", "grey", 0, 0);
        addToGridPane(gridPane, "实际应付", "grey", 0, 1);
        ...
        addToGridPane(gridPane, "本年度采购合计", "orange", 0, 14);


        addToGridPane(gridPane, "采购商名称", "grey", 0, 16);
        addToGridPane(gridPane, "供应商付款总结", "grey", 0, 29);
...
//                addToGridPane(gridPane, dao.getSumOrDefaultZero("actual"), "grey", j + 1, 1);
                addToGridPaneLink(gridPane,
                        dao.getSumOrDefaultZero("actual"),
                        "grey",
                        j + 1,
                        1,
                        MainPaneList.OUTCOME_ANNUAL,
                        FXUtils.ps(
                                new Pair<>("year", year),
                                new Pair<>("sid", dao.getUid()))
                );
            ...
            addToGridPane(gridPane2, "每月采购总结", "orange", 0, 1);
            addToGridPane(gridPane2, "每月付款总结", "orange", 0, 2);
            addToGridPane(gridPane2, "年度总采购", "orange", 0, 3);
            addToGridPane(gridPane2, "年度总付款", "orange", 0, 4);
    ...
    public StringBinding getTitle() {
        return FXWidgetUtil.sBinding("应付统计总表");
    }
}
